...
public class LauncherProvider extends ContentProvider {
    ...
    public static final int GAMES_FOLDER_RESID=R.string.title_folder_games;
    public static final int APPS_FOLDER_RESID=R.string.title_folder_recommend_app;
  //topwise zyf add end

    private static final int DATABASE_VERSION = 3055;

    static final String AUTHORITY = "com.tpw.homeshell.settings";
    ...
    private static final String ACTION_APPWIDGET_DEFAULT_WORKSPACE_CONFIGURE =
            "com.tpw.homeshell.action.APPWIDGET_DEFAULT_WORKSPACE_CONFIGURE";
    ...
    synchronized public void loadDefaultFavoritesIfNecessary(int origWorkspaceResId) {
        ...
        if (!mDbAgedMode && sp.getBoolean(DB_CREATED_BUT_DEFAULT_WORKSPACE_NOT_LOADED, false)) {
            ...
            if (workspaceResId == 0) {
                if (Utils.isForCMCC()) {
                    Log.e(TAG, "build for cmcc");
                    workspaceResId = sp.getInt(DEFAULT_WORKSPACE_RESOURCE_ID, R.xml.default_workspace_for_cmcc);
                }
                //added by qinjinchuan topwise for default topwise widget workspace configuration
                else if(TopwiseConfig.TOPWISE_WIDGET_WORKSPACE){
                    if(TopwiseConfig.YUNOS_CTA_SUPPORT){
                        workspaceResId = sp.getInt(DEFAULT_WORKSPACE_RESOURCE_ID, R.xml.default_workspace_for_topwise_widget_cta);
                    }else{
                        workspaceResId = sp.getInt(DEFAULT_WORKSPACE_RESOURCE_ID, R.xml.default_workspace_for_topwise_widget);
                    }
                }
                //added by qinjinchuan topwise for default topwise widget workspace configuration
                else {
	    ...
	    //start:fanxiangjian,topwise widget
      	    if(TopwiseConfig.TOPWISE_WIDGET_WORKSPACE){
		mOpenHelper.loadFavorites(mOpenHelper.getWritableDatabase(), R.xml.aged_workspace_for_topwise_widget);
      	    }else{
            	mOpenHelper.loadFavorites(mOpenHelper.getWritableDatabase(), R.xml.aged_workspace);
    	    }
	    //end:fanxiangjian,topwise widget
            editor.commit();
        } else {
            return;
        }
...
//topwise zyf add for hide folder
    public long loadFixedFolder(ScreenPosition sposition)
    {
    	String zpflag="NG";//android.os.SystemProperties.get("persist.sys.zp.flag1", "NG");
        Log.d("zyftest","................zpflag = "+zpflag);
        long id=-1;
        if(zpflag.equals("OK"))
        {
        	if(sposition!=null&&mOpenHelper!=null)
        	{
	        	Log.d(TAG,"loadFixedFolder.....0000........... "+sposition.s+" , "+sposition.x+" , "+sposition.y);
	        	id=mOpenHelper.loadFixedFolder(mOpenHelper.getWritableDatabase(),LauncherSettings.Favorites.CONTAINER_DESKTOP,
	        			sposition.s,sposition.x,sposition.y,LauncherSettings.Favorites.INTNET_GAMES,LauncherProvider.GAMES_FOLDER_RESID);
        	}
        }
        return id;
    }
//topwise zyf add for hide folder end
    ...
    public static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        private static final String TAG_FIXEDFOLDER = "fixedfolder";
        //topwise zyf add end
        ...
        public void onCreate(SQLiteDatabase db) {
            ...
            if (!convertDatabase(db)) {
                ...
                if (LOGD) Log.d(TAG, "set flag to load default workspace later");
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) { 
            ...
            try {
               ...
               if (version < 3040) {
                    ...
                    intentStr = "#Intent;action=android.intent.action.MAIN;" +
                            "category=android.intent.category.LAUNCHER;launchFlags=0x10200000;" +
                            "component=com.yunos.easyclone/.MainActivity;end";
                ...
                Log.w(TAG, "Destroying all old data. " + e.toString());
                db.execSQL("DROP TABLE IF EXISTS " + TABLE_FAVORITES);
                onCreate(db);
            }
        }
                ...
                while (((type = parser.next()) != XmlPullParser.END_TAG ||
                    ...
                    if (TAG_FAVORITE.equals(name)) {
                        ...
                        added = id >= 0;
                    		...
                    		//topwise zyf add for fixedfolder
                    		||TAG_FIXEDFOLDER.equals(name)
                    		//topwise zyf add end
                    		) {
                        String title;

                      //topwise zyf add for exapp
							String strclassName = a.getString(R.styleable.Favorite_className);
                        Log.d("zyfonline","loadFavorites : .............. strclassName = "+strclassName);
                        if(strclassName!=null)
                        {
	                    	if(strclassName.equals(LauncherSettings.Favorites.INTNET_GAMES))
	                    	{
	                    		values.put(LauncherSettings.Favorites.INTENT, LauncherSettings.Favorites.INTNET_GAMES);
	                    	}
	                    	else if(strclassName.equals(LauncherSettings.Favorites.INTNET_APPS))
	                    	{
	                    		values.put(LauncherSettings.Favorites.INTENT, LauncherSettings.Favorites.INTNET_APPS);
	                    	}
                        }
                    	//topwise zyf add for exapp end
                        //zyf test end
                        ...
                        if (titleResId != -1) {
                        	...
                        	Log.d("zyfonline","loadFavorites : titleResId = "+titleResId
                        			+" , "+LauncherProvider.APPS_FOLDER_RESID
                        			+" , "+LauncherProvider.GAMES_FOLDER_RESID);
                        	
                        	if(titleResId==LauncherProvider.APPS_FOLDER_RESID)
                        	{
                        		values.put(LauncherSettings.Favorites.INTENT, LauncherSettings.Favorites.INTNET_APPS);
                        	}
                        	else if(titleResId==LauncherProvider.GAMES_FOLDER_RESID)
                        	{
                        		values.put(LauncherSettings.Favorites.INTENT, LauncherSettings.Favorites.INTNET_GAMES);
                        	}
                        	*/
                            ...
                            Log.d("zyfonline","loadFavorites : title = "+title);
                        } else {
                            title = mContext.getResources().getString(R.string.folder_name);
                            Log.d("zyfonline","loadFavorites : title = "+title);
                        }
                        values.put(LauncherSettings.Favorites.TITLE, title);
                        //topwise zyf modify for fixedfolder
                        long folderId ;
                        if(TAG_FOLDER.equals(name))
                        {
                        	folderId = addFolder(db, values);
                        }
                        else
                        {
                        	folderId = addFixedFolder(db, values);
                        }
                        
                      //topwise zyf modify end
                        added = folderId >= 0;
                        if (mDbAgedMode) {
...
//topwise zyf add for hide folder
        private long loadFixedFolder(SQLiteDatabase db,int container,int screen,int x,int y,String strintent,int titleResId) {
        	ContentValues values = new ContentValues();
            values.clear();
            values.put(LauncherSettings.Favorites.CONTAINER, container);
            values.put(LauncherSettings.Favorites.SCREEN, screen);
            ...
            String title;
             ...
             values.put(LauncherSettings.Favorites.INTENT, strintent);
        	...
        	long folderId = addFixedFolder(db, values);
      ...
      //topwise zyf add for fixedfolder
        private long addFixedFolder(SQLiteDatabase db, ContentValues values) {
            values.put(Favorites.ITEM_TYPE, Favorites.ITEM_TYPE_FIXED_FOLDER);
            values.put(Favorites.SPANX, 1);
            values.put(Favorites.SPANY, 1);
            long id = generateNewId();
            values.put(Favorites._ID, id);
