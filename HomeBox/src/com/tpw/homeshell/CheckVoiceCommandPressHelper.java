...
public class CheckVoiceCommandPressHelper {
    ...
    public void postCheckVoiceCommand(final View view) {
        Log.d(TAG, "sxsexe------> postCheckVoiceCommand isVoiceSwitchOn : " + isVoiceSwitchOn() 
                + " mMessenger " + mMessenger
                 + " PUSH_TO_TALK_SUPPORT " + PUSH_TO_TALK_SUPPORT
                 + " view.tag " + (view == null ? null : view.getTag()));
        if(!PUSH_TO_TALK_SUPPORT || !isVoiceSwitchOn()) {
            return;
        }
        ...
        buildHotZone(view);
        if (mMessenger == null) {
            // if service is not connected,re-connect it
            if (mLauncher != null) {
    ...
    public static void checkEnvironment() {
        if (Utils.isLowMemoryDevice()) {
            PUSH_TO_TALK_SUPPORT = false;
        } else if (Utils.isYunOSInternational()) {
            PUSH_TO_TALK_SUPPORT = false;
        } else {
            checkLanguage();
        }
