...
public class CellLayout extends ViewGroup {
    ...
    private String mWidgetPagePackageName;
    ...
    private void computePaddings() {
        ...
        mCellWidth = ConfigManager.getCelllayoutCellWidth(this.getId() == R.id.layout_hotseat);
        mCellHeight = ConfigManager.getCelllayoutCellHeight(this.getId() == R.id.layout_hotseat);

        Resources res = getResources();
        int layoutH = res.getDisplayMetrics().heightPixels;
        layoutH = layoutH
                - getResources().getDimensionPixelSize(
                        R.dimen.droptarget_height);
        // for 3*3 layout
        if (AgedModeUtil.isAgedMode()) {
            layoutH = layoutH
                    - res.getDimensionPixelSize(R.dimen.workspace_top_padding)
        ...
        layoutH = layoutH - this.getPaddingTop() - this.getPaddingBottom();// res.getDimensionPixelSize(R.dimen.cell_layout_bottom_padding);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mFolderOuterRings.size(); i++) {
            ...
            int width = (int) fra.getOuterOutlineWidth();
            int height = (int) fra.getOuterOutlineHeight();
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (isHideseatOpen) {
            ...
            for (int i = 0; i < N; i++) {
                Bitmap clipWidget = mCachedBmps.get(i);
                Rect rect = mCachedRects.get(i);
     ...
     */
    public void drawShortcutsAndWidgetsOnCanvas(Canvas canvas) {
        canvas.save();
        ...
        try {
            ...
            // getDrawingCache() can easily lead to OOM exception. Due to the framework
            // implementation, the OOM exception won't be thrown out. So we have to
            // catch all kinds possible exceptions here.
            Log.w(TAG, "drawShortcutsAndWidgetsOnCanvas failed. (probably OOM)", ex);
        }
        canvas.restore();
    }
    ...
    public boolean addViewToCellLayout(View child, int index, int childId, LayoutParams params,
        ...
        if (lp.cellX >= 0 && lp.cellX <= mCountX - 1 && lp.cellY >= 0 && lp.cellY <= mCountY - 1) {
            ...
            Log.d(TAG, "sxsexe------> add " + info + " mode " + mMode);
            mShortcutsAndWidgets.addView(child, index, lp);
            if(this == mLauncher.getHotseat().getCellLayout()) {
                mLauncher.getHotseat().initViewCacheList();
        } else {
            if (info != null && mLauncher.isContainerHotseat(info.container)) {
    ...
    void cellToPoint(int cellX, int cellY, int[] result) {
        ...
        if (isHideseatOpen) {
            if (cellY >= HIDESEAT_CELLY) {
                result[1] += ConfigManager.getHideseatHeight();
            }
        }
    }
    ...
    void buildChildrenCacheIfNeed() {
        ...
        for (int i = 0; i < N; i++) {
            ...
            ItemInfo info = (ItemInfo) child.getTag();
            if (info.cellY < HIDESEAT_CELLY && info.cellY + info.spanY > HIDESEAT_CELLY &&
    ...
    private void buildChildCache(View child, boolean rebuildCache) {
        ...
        Bitmap bmp = Bitmap
                .createBitmap(W, H + ConfigManager.getHideseatHeight(), Config.ARGB_8888);
        ...
        dst = new Rect(0, top + ConfigManager.getHideseatHeight(), W, H
...
//        Log.d(TAG, "sxsexe99----> onMeasure begin mOriginalHeightGap " + mOriginalHeightGap 
//                + " isHotseat " + mIsHotseat + " isHideseatMode " + isHideseatMode
//                + " this " + this
...
//            mHeightGap = Math.min(mMaxGap,numHeightGaps > 0 ? (vFreeSpace / numHeightGaps) : 0);
        ...
        if (widthSpecMode == MeasureSpec.AT_MOST) {
            ...
            if (isHideseatOpen) {
                newHeight += ConfigManager.getHideseatHeight();
            }
        ...
        mCellLayoutHeight = newHeight;
        mCellLayoutWidth = newWidth;
        setMeasuredDimension(newWidth, newHeight);
...
//                + " heightSpecSize " + heightSpecSize + " mOriginalHeightGap " + mOriginalHeightGap);
    ...
    void visualizeDropLocation(View v, Bitmap dragOutline, int originX, int originY, int cellX,
            ...
            boolean toHotseat, boolean fromHotseat, int touchX) {
        if (isLayoutRtl()) {
        ...
        if (toHotseat && mLauncher.getHotseat().isFull()) {
            // if hotseat is full , don't draw dragOutline
            return;
        }
        ...
        if (cellX != oldDragCellX || cellY != oldDragCellY) {
            ...
            if(toHotseat) {
                mLauncher.getHotseat().touchToPoint(touchX, topLeft, fromHotseat, toHotseat);
            }
            int left = topLeft[0];
            int top = topLeft[1];
            /* casue no need to offset the dragOutline
             * if (v != null && dragOffset == null) {
    ...
    /** @see ReorderAnimationListener */
    void setReorderAnimationListener(ReorderAnimationListener listener) {
    ...
    */
    ...
    public static int[] rectToCell(Resources resources, int width, int height, int[] result) {
        ...
        spanX = Math.min(spanX, LauncherModel.getCellCountX());
        spanY = Math.min(spanY, LauncherModel.getCellCountY());
            ...
            if (isLockedToGrid) {
                ...
                if (isHideseatMode && !ignoreHideseat) {
                    if (myCellY >= HIDESEAT_CELLY) {
                        y += ConfigManager.getHideseatHeight();
                    }
                }
            }
        }
...
//                : null;
//    }
    // remove drag listener when finalizing
    @Override
    protected void finalize() throws Throwable {
        if ((mLauncher != null) && (mDragEnforcer != null) && mLauncher.getDragController() != null) {
            mLauncher.getDragController().removeDragListener(mDragEnforcer);
        }
        super.finalize();
    }
        ...
        if (mEditBtnContainer == null) {
            ...
            delBtn.setOnClickListener(new OnClickListener(){
                ...
                public void onClick(View view) {
                    mLauncher.getWorkspace().deleteEmptyScreen(CellLayout.this);
                }
    ...
    public boolean isWidgetPage() {
        return (mWidgetPagePackageName != null);
    }

    public void setWidgetPagePackageName(String widgetPagePackageName) {
        mWidgetPagePackageName = widgetPagePackageName;
    }

    public String getWidgetPagePackageName() {
        return mWidgetPagePackageName;
    }
    ...
    private void onDoubleClick(int[] cellPosition) {
        final int screen = mLauncher.getWorkspace().indexOfChild(CellLayout.this);
        ...
        if (!HomeShellSetting.getFreezeValue(mLauncher)) {
        	//start modify by huangxunwan for cofig double click to rearrange celllayout icon 20150608
        	if (TopwiseConfig.SUPPORT_DOUBLE_CLICK_REARRANGE){
        		reOrderIcons();
        	}
    ...
    private void reOrderIcons() {
        ...
        final int screen = mLauncher.getWorkspace().indexOfChild(CellLayout.this);
        final int container = Favorites.CONTAINER_DESKTOP;
        Map<String, String> param = new HashMap<String, String>();
        param.put("screen", String.valueOf(screen + 1));
        UserTrackerHelper.sendUserReport(
                UserTrackerMessage.MSG_DOUBLECLICK_ARRANGE, param);
                    ...
                    if (mOccupied[xIndex][yIndex]) {
                        ...
                        animY.addListener(new AnimatorListenerAdapter() {
                            ...
                            public void onAnimationEnd(Animator animation) {
                                ...
                                int childId = LauncherModel.getCellLayoutChildId(container, screen,
