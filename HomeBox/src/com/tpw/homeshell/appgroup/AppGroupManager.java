package com.tpw.homeshell.appgroup;
...
public class AppGroupManager {
    ...
    public static final String URL = "http://apps.tpw.com/api.htm";
    ...
    //avoid server data less than min size
...
//            cb.onResult(fi, folderName);
    ...
    public String requestSingleAppFromServer(String pkgName) {
        if (!Utils.isNetworkConnected() || TextUtils.isEmpty(pkgName)) {
            return "";
        }
    ...
    public void loadAppGroupInfosFromServer() {
        if (!Utils.isNetworkConnected() || sOff) {
            mIsLoading = false;
            return;
        }
        new Thread(new Runnable() {
    ...
    private String getCatIdByPkg(String pkgName) {
    ...
    private CategoryInfo getCatInfoByCatID(String catID) {
