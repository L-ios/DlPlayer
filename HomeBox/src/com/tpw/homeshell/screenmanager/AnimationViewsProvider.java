...
class AnimationViewsProvider {
    ...
    void open() {
        ...
        mOpened = true;
        int screens = mLauncher.getWorkspace().getChildCount();
        mCurrentScreen = mLauncher.getWorkspace().getCurrentPage();
        for (int i = 0; i < screens; i ++) {
            CellLayout layout = (CellLayout) mLauncher
                    .getWorkspace().getChildAt(i);
            if (layout == null) {
        ...
        mHotSeat = mLauncher.getHotseat().getLayout();
        mHotSeatX = mLauncher.getHotseat().getX() + mHotSeat.getX();
        mHotSeatY = mLauncher.getHotseat().getY() + mHotSeat.getY();
        mLauncher.getHotseat().removeView(mHotSeat);
    ...
    void close() {
        ...
        mHotSeat.setX(0);
        mHotSeat.setY(0);
        mLauncher.getHotseat().addView(mHotSeat);
        mViews.clear();
        mParams.clear();
        mOpened = false;
    }
