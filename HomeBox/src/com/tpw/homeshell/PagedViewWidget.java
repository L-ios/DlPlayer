...
public class PagedViewWidget extends LinearLayout {
    ...
    /*public void applyFromGadgetInfo(GadgetInfo info,
            int maxWidth, int[] cellSpan, WidgetPreviewLoader loader) {
        mIsAppWidget = true;
        mInfo = info;
        final ImageView image = (ImageView) findViewById(R.id.widget_preview);
        ...
        final TextView name = (TextView) findViewById(R.id.widget_name);
        name.setText(ThemeUtils.getGadgetName(getContext(), info));
        final TextView dims = (TextView) findViewById(R.id.widget_dims);
        if (dims != null) {
            int hSpan = Math.min(cellSpan[0], LauncherModel.getCellCountX());
            int vSpan = Math.min(cellSpan[1], LauncherModel.getCellCountY());
    ...
    void applyPreview(FastBitmapDrawable preview, int index) {
        ...
        if (preview != null) {
            ...
            if (mIsAppWidget) {
                ...
                int centerwidth = (imageSize[0] - preview.getIntrinsicWidth()) / 2;
                int centerHeight = (imageSize[1] - preview.getIntrinsicHeight()) / 2;
                image.setPadding(mOriginalImagePadding.left + centerwidth,
                        mOriginalImagePadding.top + centerHeight,
